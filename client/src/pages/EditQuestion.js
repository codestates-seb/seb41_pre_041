...
const EditQuestion = () => {
  ...
  const getQuestion = async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/questions/{question-id}`);
      setQData(res.data);
    } catch (err) {
      console.error(err);
    }
  ...
  const editSubmit = async () => {
    try {
      await axios.patch(`${process.env.REACT_APP_API_URL}/api/questions/{question-id}`, {
        title,
        content: post,
      });
    } catch (err) {
